// LAB3_BST.C - Lab 03 - Muhammad Saad, Iqbal
...
{ 
    ...
    int heightL;
    if (root != NULL)	{
        heightL=height(root->leftChild);
        heightR=height(root->rightChild);
    }
    nodeheight = (heightL>heightR) ? heightL : heightR ;
    return nodeheight + 1;
...
{
    if (root->leftChild && root->leftChild->key == k) return root;

    if (root->rightChild && root->rightChild->key == k) return root;

    if (root->leftChild && root->key > k)
        return findParentHelper(k, root->leftChild);
    if (root->rightChild && root->key < k)
...
//	p	- points to parent of node to be deleted.
{
    ...
    if (n->leftChild != NULL) {			// there is left child
        ...
        if (p->rightChild == n) p->rightChild = deleteNode->leftChild;
        if (p->leftChild == n) p->leftChild = deleteNode->leftChild;
        free(n);

    } else if (n->rightChild) {			// there is a right child

        if (p->rightChild == n) p->rightChild = deleteNode->rightChild;
        if (p->leftChild == n) p->leftChild = deleteNode->rightChild;
        free(n);

    } else	{							// no children

        if (p->rightChild == n) p->rightChild = NULL;
