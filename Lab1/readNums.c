// readnumsv6.c - Lab 01 - Muhammad Saad, Iqbal
...
int delete_data(int d) {
    struct node *ptemp;     // created a new, temporary, node pointer to help traverse
    struct node *pprev;     // created node pointer to keep track of previous pointer
    ptemp = phead;          // temporary pointer set to pointer head's pointee
    int found = 0;          // variable to control loop
    while (!found) {
        // if the desired value is found
        if (ptemp->datavalue == d) {
            if (ptemp == phead) {
                // if the desired value is at head position, then head is shifted
                phead = ptemp->pnextnode;
                // memory for node is freed
                free(ptemp);
            } else {
                // the previous node is linked with next usable node
                pprev->pnextnode = ptemp->pnextnode;
                // memory for node is freed
                free(ptemp);
            }
            // desired value is found, while loop will not run anymore
            found=1;
        }
        // the previous pointer is set
        pprev = ptemp;
        // the pointer is shifted to next node to traverse list
...
void main (void)
{
    ...
    printf ("To delete a number, enter letter D and any number.\n");
